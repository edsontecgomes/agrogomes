...
function obterCoordenadasTalhao(
  talhao: TalhaoMapCanvasV2Props["talhoes"][number],
): LatLng[] {
  if (
...
function calcularAreaHa(
  ...
  const areaM2 =
    googleMaps.maps.geometry.spherical.computeArea(
      caminho,
    );
...
export function TalhaoMapCanvasV2({
  ...
  useEffect(() => {
    ...
    async function inicializarMapa() {
      try {
        ...
        const zoom =
          coordenadasPrimeiroTalhao.length >= 3
            ? 15
            : 4;
  ...
  useEffect(() => {
    ...
    if (
      ...
      desenhoPolylineRef.current.setMap(
        null,
      );
    ...
    if (
      ...
      desenhoPolygonRef.current.setMap(
        null,
      );
  ...
  useEffect(() => {
    ...
    if (
      ...
      googleMaps.maps.event.removeListener(
        clickListenerRef.current,
      );
  ...
  useEffect(() => {
    ...
    const limites =
      new googleMaps.maps.LatLngBounds();
      ...
      {mode === "desenho" && !loading && !erro && (
        <div className="pointer-events-none absolute left-4 top-4 rounded-2xl bg-slate-950/85 px-4 py-3 text-white shadow-lg backdrop-blur">
          ...
          <p className="mt-2 text-xs font-bold text-white">
            Pontos: {coordenadasDesenho.length}
          </p>
        </div>
      )}
