...
interface OrdemCardProps {
  ...
  estoque: Estoque[];
  canManage: boolean;
  activeExecs: ExecucaoServico[];
  myExec?: ExecucaoServico | null;
...
export function OrdemCard({
  ...
  const nomeTalhao =
    talhoes.find(
      talhao => talhao.id === ordem.talhaoId
    )?.nome || 'Talhão não identificado';
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div className="space-y-1">
            ...
            {ordem.descricao && (
              <p className="text-slate-600 text-sm italic line-clamp-2 mt-2">
                {ordem.descricao}
              </p>
            )}
          </div>
      ...
      {confirmFinish && myExec && (
        <div className="border-t border-slate-100 bg-slate-50 p-6 space-y-4 animate-in slide-in-from-top-2 duration-200">
          ...
          <p className="text-sm text-slate-600">
            Sua execução será encerrada e os dados serão
            salvos. Se houver outros operadores, eles
            continuarão ativos.
          </p>
...
function StatusBadge({
  ...
  if (status === 'pendente') {
    return (
      <span className="px-3 py-1 bg-amber-100 text-amber-800 text-[10px] font-bold rounded-full flex items-center gap-1 uppercase tracking-wider">
  ...
  if (status === 'em_execucao') {
    return (
      <span className="px-3 py-1 bg-blue-500 text-white text-[10px] font-bold rounded-full flex items-center gap-1 uppercase tracking-wider shadow-sm">
  ...
  if (status === 'parcial') {
    return (
      <span className="px-3 py-1 bg-slate-300 text-slate-800 text-[10px] font-bold rounded-full flex items-center gap-1 uppercase tracking-wider">
  ...
  if (status === 'finalizada') {
    return (
      <span className="px-3 py-1 bg-emerald-600 text-white text-[10px] font-bold rounded-full flex items-center gap-1 uppercase tracking-wider">
        <CheckCircle2 className="w-3 h-3" />
        Finalizada
      </span>
    );
