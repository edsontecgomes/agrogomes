...
export type UserRole = 'admin' | 'produtor' | 'gerente' | 'colaborador' | 'operador' | 'system_admin';
...
export interface Usuario {
  ...
  nome: string;
  role: UserRole;
  farmId?: string; // For Operadores/Gerentes assigned to a specific farm
  primaryFarmId?: string; // For Produtores to remember last selection
  producerId?: string; // If the user is linked to a producer (for collab/op)
  createdAt: Date;
}
...
export interface ChecklistTemplate {
  id: string;
  farmId: string;
  tipoOperacao: 'Plantio' | 'Pulverizacao' | 'Adubacao' | 'Colheita' | 'Outros';
  nome: string;
  itens: ChecklistItem[];
  ativo: boolean;
  createdAt: Date;
...
export interface Talhao {
  ...
  nome: string;
  farmId: string;
  geometria?: any; // GeoJSON Polygon
  area: number; // Em hectares
  areaHa?: number; // Em hectares
  coordenadas?: { lat: number; lng: number }[];
  cor?: string; // Cor personalizada do talhão
  producerId?: string;
  createdAt: Date;
  updatedAt?: Date;
}
...
export interface PlanoManutencao {
  ...
  tipoManutencao: 'Troca de Óleo' | 'Troca de Filtro' | 'Lubrificação' | 'Revisão' | 'Pneus' | 'Correias' | 'Sistema Hidráulico' | 'Outros';
...
export interface PecaManutencao {
  ...
  produtoEstoqueId?: string; // Vinculo ao estoque (references ProdutoEstoque)
  equipamentosCompativeis?: string[]; // Referencias a Equipamento ID
  manutencoesAssociadas?: string[]; // Referencias a ManutencaoExecutada ID
  ativo: boolean;
  createdAt: any;
  updatedAt?: any;
}
