...
import { useExecucoesServico, useMinhasExecucoesAtivas } from '../hooks/useServicos';
import { Talhao } from '../types';
import { PlayCircle, X, MapPin, AlertCircle, Navigation } from 'lucide-react';

interface GeofenceSuggesterProps {
  farmId: string;
}

export function GeofenceSuggester({ farmId }: GeofenceSuggesterProps) {
  const { suggestedOrdem, location, currentTalhao, dismissSuggestion } = useTalhaoGeofence(farmId);
  const { iniciarExecucao } = useExecucoesServico(suggestedOrdem?.id || null);
  const { execucoesAtivas } = useMinhasExecucoesAtivas(farmId);

  const handleStart = async () => {
    if (!suggestedOrdem || !currentTalhao) return;
    
    await iniciarExecucao(farmId, currentTalhao.id, location ? { 
      lat: location.lat, 
      lng: location.lng, 
  ...
  return (
    <>
      <AnimatePresence mode="wait">
        {suggestedOrdem && !isTrackingActive && (
            ...
            >
              <div className="p-6">
                ...
                <p className="text-slate-600 mb-6 text-sm leading-relaxed">
                  Você entrou no talhão <span className="font-bold text-slate-900">{currentTalhao?.nome}</span>. 
                  Existe uma ordem de <span className="font-bold text-slate-900">{suggestedOrdem.tipoOperacao}</span> pendente. 
                  Confirmar início do trabalho?
                </p>
              ...
              {location && location.accuracy > 20 && (
                <div className="bg-amber-50 px-6 py-2.5 flex items-center gap-2 border-t border-amber-100">
                  ...
                  <span className="text-[10px] text-amber-700 font-bold uppercase tracking-wider">
                    Sinal GPS instável ({Math.round(location.accuracy)}m)
                  </span>
                </div>
              )}
            </motion.div>
      ...
      {/* GPS Status & Tracking Indicators */}
      <div className="fixed bottom-24 right-6 z-40 flex flex-col items-end gap-2 pointer-events-none">
        {isTrackingActive && (
          ...
          >
            ...
            <span className="text-xs font-black uppercase tracking-widest">Tracking Ativo</span>
          </motion.div>
        )}
